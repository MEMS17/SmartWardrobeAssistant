...
1. **Accueil (HomeScreen)**
   ...
   - Utilisateur clique sur "MA TENUE ✨"
...
### Fichier `.env` (à créer localement, JAMAIS commiter)
...
### Fichier `.env.example` (modèle, peut être committé)
...
- Le fichier `.gitignore` protège automatiquement `.env`
- Si une clé a été exposée, régénérez-la immédiatement
- Les variables d'env sont chargées via `react-native-dotenv`
...
3. Créer une nouvelle clé API dans "Get API Key"
4. Copier la clé et la mettre dans `.env`

**Note:** L'API Gemini offre un usage gratuit avec des limitations.
...
```bash
# Remplacer YOUR_API_KEY par votre clé Gemini
    ...
    "contents": [
      {
        "parts": [
          {
            "text": "Suggest an outfit in JSON format for temperature 15°C and rainy weather"
          }
        ]
      }
    ]
...
- Babel configuré pour les variables d'env
- `.gitignore` protège les secrets
- `.env.example` fourni pour la documentation
...
2. Ajouter votre clé API Gemini dedans
3. Testez l'app en ajoutant des vêtements dans l'armoire
4. Allez dans "MA TENUE ✨" pour voir la recommandation
5. Vérifiez que les vêtements recommandés existent dans votre armoire
6. Rafraîchissez pour obtenir une autre recommandation
